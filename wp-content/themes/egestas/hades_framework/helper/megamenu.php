...
class H_Menu_Frontend extends Walker_Nav_Menu  {
	...
	function start_lvl(&$output, $depth) {
		...
		$output .= "\n$indent<{$element} class=\"sub-menu  clearfix  $widget_class \" data-filter='bg rbr' data-list-filter='bbr' data-link-filter='cr crh'>\n";
	}
	...
	function start_el(&$output, $item, $depth, $args) {
	  ...
	  if(!$this->isMegaMenu) {	 
		...
		$item_output .= "<span class='menu-subtitle'> $subtitle </span>".'</a> ';
	  ...
	  else if($this->menutype=="column")
	  {
		...
		if($depth==1)
		{
			...
			$item_output .= '<h6 class="custom-font">';
		}
		else
		$item_output .= '<a'. $attributes .'>';
...
class H_MegaMenu_Nav_Menu extends Walker_Nav_Menu {
	...
	function start_el(&$output, $item, $depth, $args) {
		...
		if ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
			...
			$title = sprintf( __('%s (Pending)','h-framework'), $item->title );
		}
